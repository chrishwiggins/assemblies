...
def generic_trans_verb(index):
    return {
        ...
        "POST_RULES": [
            ...
            FiberRule(INHIBIT, VERB, DEP_CLAUSE, 0),
        ],
    }
...
def generic_determinant(index):
    return {
        "index": index,
        "PRE_RULES": [AreaRule(DISINHIBIT, DET, 0), FiberRule(DISINHIBIT, LEX, DET, 0)],
        "POST_RULES": [
...
def generic_adjective(index):
    return {
        "index": index,
        "PRE_RULES": [AreaRule(DISINHIBIT, ADJ, 0), FiberRule(DISINHIBIT, LEX, ADJ, 0)],
        "POST_RULES": [
...
def generic_russian_verb(index):
    return {
        ...
        "POST_RULES": [FiberRule(INHIBIT, LEX, VERB, 0)],
    }
...
def generic_russian_nominative_noun(index):
    return {
        ...
        "POST_RULES": [FiberRule(INHIBIT, LEX, NOM, 0)],
    }
...
def generic_russian_accusative_noun(index):
    return {
        ...
        "POST_RULES": [FiberRule(INHIBIT, LEX, ACC, 0)],
    }
...
def generic_russian_dative_noun(index):
    return {
        ...
        "POST_RULES": [FiberRule(INHIBIT, LEX, DAT, 0)],
    }
...
class ParserBrain(brain.Brain):
    ...
    def getActivatedFibers(self):
        # Prune activated_fibers based on the readout_rules
...
class ParserDebugger:
    ...
    def peak(self):
        ...
        for area, num_test_projects in remove_map.items():
            self.b.area_by_name[area].winners = self.b.area_by_name[area].saved_winners[
                0
            ]
            self.b.area_by_name[area].w = self.b.area_by_name[area].saved_w[
                -num_test_projects - 1
            ]
    ...
    def read_out(area, mapping):
        ...
        for to_area in to_areas:
            ...
            if to_area == DEP_CLAUSE:
                ...
                b.project({}, {VERB: [LEX, SUBJ]})
                dep_verb = b.getWord(LEX)
                dependencies.append([this_word, dep_verb, "DEP-VERB"])
                b.project({}, {SUBJ: [LEX]})
                dep_verb_subj = b.getWord(LEX)
