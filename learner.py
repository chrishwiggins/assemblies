...
def lexicon_sizes_experiment(
    ...
    LEX_n=100000,
    beta=0.06,
    extra_context_areas=0,
    extra_context_model="B",
    use_extra_context=False,
    extra_context_area_k=20,
    repeat=1,
    output_file=None,
):
    results = {}
    ...
    for n in range(start, end + 1):
        ...
        for _ in range(repeat):
            brain = LearnBrain(
                ...
                num_verbs=n,
                beta=beta,
                extra_context_areas=extra_context_areas,
                extra_context_model=extra_context_model,
            )
            brain.no_print = True
...
def single_word_tutoring_exp(
    ...
    while lex_size <= lex_size_end:
        ...
        for _ in range(repeat):
            ...
            brain.no_print = True
            num_words_needed = brain.train_experiment_randomized_with_tutoring(
                single_word_frequency=single_word_frequency,
                testing_increment=testing_increment,
            )
...
class LearnBrain(brain.Brain):
    def __init__(
        ...
        self.add_explicit_area(
            VISUAL, self.num_nouns * CONTEXTUAL_k, CONTEXTUAL_k, beta
        )
        ...
        if self.extra_context_model == "B" and self.extra_context_areas > 0:
            for i in range(extra_context_areas):
                ...
                self.add_explicit_area(
                    extra_context_area_name,
                    self.lex_size * extra_context_area_k,
                    extra_context_area_k,
                    beta,
                )
            self.extra_context_map = {}
            for word_index in range(self.lex_size):
                self.extra_context_map[word_index] = random.randint(
                    0, self.extra_context_areas - 1
                )
    ...
    def parse_indexed_sentence(self, noun_index, verb_index, order="NV"):
        ...
        if (
            ...
            and self.sentences_parsed > self.extra_context_delay
        ):
            if self.extra_context_model == "A":
                ...
                noun_extra_context_area_name = self.get_extra_context_area_name(
                    noun_extra_context_index
                )
                verb_extra_context_index = self.extra_context_map[verb_index]
                verb_extra_context_area_name = self.get_extra_context_area_name(
                    verb_extra_context_index
                )
    ...
    def train_experiment_randomized(
        self, max_samples=500, increment=1, start_testing=0, use_extra_context=False
    ):
        # self.extra_context_areas = 0
        for i in range(0, max_samples):
            ...
            if (
                (i > start_testing)
                and (i % increment == 0)
                and self.test_all_words(use_extra_context=use_extra_context)
            ):
    ...
    def train_experiment_randomized_with_tutoring(
        self,
        max_samples=500,
        testing_increment=1,
        start_testing=0,
        single_word_frequency=5,
    ):
        num_words = 0
        for i in range(1, max_samples + 1):
            ...
            if (
                (i > start_testing)
                and (i % testing_increment == 0)
                and self.test_all_words()
            ):
    ...
    def test_all_words(self, use_extra_context=False):
        for word_index in range(self.num_nouns + self.num_verbs):
            if (
                self.testIndexedWord(
                    word_index, use_extra_context=use_extra_context, no_print=True
                )
                != word_index
            ):
                return False
    ...
    def testIndexedWord(
        self, word_index, min_overlap=0.75, use_extra_context=False, no_print=False
    ):
        self.disable_plasticity = True
        self.area_by_name[PHON].unfix_assembly()
    ...
    def get_input_from(self, from_area, to_area):
        ...
        if (not from_winner_indices) or (not to_winner_indices):
            return 0
        total_input = (
            (self.connectomes[from_area][to_area])[from_winner_indices][
                :, to_winner_indices
            ]
        ).sum()
        return total_input
...
class SimpleSyntaxBrain(brain.Brain):
        ...
        for noun_index in [0, 1]:
            ...
            for _ in range(proj_rounds):
                self.project(
                    {},
                    {
                        NOUN_VERB: [NOUN_VERB, VISUAL, CORE],
                        VISUAL: [NOUN_VERB],
                        CORE: [NOUN_VERB],
                    },
                )
        ...
        for verb_index in [2, 3]:
            ...
            for _ in range(proj_rounds):
                self.project(
                    {},
                    {
                        NOUN_VERB: [NOUN_VERB, MOTOR, CORE],
                        MOTOR: [NOUN_VERB],
                        CORE: [NOUN_VERB],
                    },
                )
    ...
    def pre_train_test(self):
        ...
        if max_winner >= (2 * self.area_by_name[NOUN_VERB].k):
            print(
                "ERROR: proecting noun core -> NOUN, VERB yielded winner in verb part"
            )
...
class LearnBrain_SimpleSyntax(LearnBrain):
    ...
    def train_cores(self, rounds=20):
        ...
        (self.connectomes[CORE][NOUN])[0 : self.CORE_k, :] = (
            np.ones(core_to_noun_shape) * updated_weight
        )

        noun_to_core_shape = ((self.connectomes[NOUN][CORE])[:, 0 : self.CORE_k]).shape
        (self.connectomes[NOUN][CORE])[:, 0 : self.CORE_k] = (
            np.ones(noun_to_core_shape) * updated_weight
        )

        core_to_verb_shape = ((self.connectomes[CORE][VERB])[self.CORE_k :, :]).shape
        (self.connectomes[CORE][VERB])[self.CORE_k :, :] = (
            np.ones(core_to_verb_shape) * updated_weight
        )

        verb_to_core_shape = ((self.connectomes[VERB][CORE])[:, self.CORE_k :]).shape
        (self.connectomes[VERB][CORE])[:, self.CORE_k :] = (
            np.ones(verb_to_core_shape) * updated_weight
        )
...
class LearnBrain_Syntax:
    ...
    def OBJECTS_train_cores(self, rounds=20):
        ...
        for i in intrans_verb_assembly:
            (self.connectomes[CORE][VERB])[
                self.CORE_k : (2 * self.CORE_k), i
            ] = updated_weight
            (self.connectomes[VERB][CORE])[
                i, self.CORE_k : (2 * self.CORE_k)
            ] = updated_weight
    ...
    def OBJECTS_train_syntax(train_rounds=20):
        ...
        (self.connectomes[SEQ][SEQ])[
            : self.SEQ_k, self.SEQ_k : (2 * self.SEQ_k)
        ] = SEQ_updated_weight
        self.activate(SEQ, 1)
        self.activate(CORE, 1)
        for _ in range(train_rounds):
        ...
        (self.connectomes[SEQ][SEQ])[
            (2 * self.SEQ_k) : (3 * self.SEQ_k), (3 * self.SEQ_k) : (4 * self.SEQ_k)
        ] = SEQ_updated_weight
        self.activate(SEQ, 3)
        self.activate(CORE, 0)
        ...
        (self.connectomes[SEQ][SEQ])[
            (3 * self.SEQ_k) : (4 * self.SEQ_k), (4 * self.SEQ_k) : (5 * self.SEQ_k)
        ] = SEQ_updated_weight
        self.activate(SEQ, 4)
        self.activate(CORE, 0)
        for _ in range(train_rounds):
